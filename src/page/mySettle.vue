    ...
    <div class="abs scroller_box">
      <div v-if="hasRepayment">
        <scroller ref="my_scroller" :on-refresh="refresh" :on-infinite="infinite" :noDataText="noDataText" :height="scrollHeight">
          <!--<div class="bg_white textCenter pt50 pb50">-->
          <!--<div class="c_333 fs32">待还金额(元)</div>-->
          <!--<div class="c_red fs80 mt5">{{noRepayMoneySum | addThousandSign}}</div>-->
...
//      Toast, XInput, Group, XButton, Cell
    ...
    mounted () {
      var _this = this
      _this.$store.commit('UPDATE_TITLE', {show: true, title: '已结清贷款'})//配置显示公共头部并且更新标题
    },
    methods: {
      ...
      lookViews (row) {
        //this.$router.push({path: '/repaymentDetail', query: {loanId: row.loanId}})
        this.$router.push({path: '/repayFinishDetail', query: {loanId: row.loanId}})
      },
...
<style lang="less" scoped>
  #mySettle {
    ...
    .no-repayment-data {
      ...
      width: 2.92rem;
      height: 2.55rem;
      background: url("../assets/images/logo_no_mySettle.png") no-repeat;
