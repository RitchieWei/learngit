...
<template>
  <div v-if="itemApplyId">
    <scroller ref="my_scroller" :height="scrollerHeight" :style="{top: `${scoreTop}px`}">
      <dl class="loan_header_cont">
        <dt>
          ...
          <span>{{loanTime}} | 共{{frequency}}期
            <!--<em class="h_orangeCircle">已逾期/第2期</em>-->
            <em class="h_orangeCircle" v-if="statusShow">{{statusShowText}}</em>
          </span>
        </dt>
        <dd>
          <span>总利息：<em>{{totalInterest | addThousandSign(2)}}元</em></span>
          <span>还款方式：<em>{{repayTyep}}</em></span>
        </dd>
        <dd>
          <!--<span>银行卡：<em><img class="bank-ico" src="./../../static/images/bank_logo/logo_ICBC.png" alt="">{{bankAmount}}</em></span>-->
          <span>银行卡：<em><img class="bank-ico" :src="`./../../static/images/bank_logo/logo_${bankCode}.png`" @error="setDefaultBankLogo">{{bankCode | bankCodeToBankName}}({{bankCardNm}})</em></span>
          <span class="agreement_btn" @click.self="showContract">借款合同</span>
        </dd>
      </dl>
    ...
    <!--  <div class="btm_cont">
        <div class="appay_amout">当前应还金额222：<span>{{totalAmount | addThousandSign(2)}}</span> 元</div>
        &lt;!&ndash;<router-link class="prepayment_btn" to="/prepayment">提前还款</router-link>&ndash;&gt;
        <li v-if="statusShow" class="prepayment_btn" style="background: #ddd;">提前还款2</li>
...
//      Toast, Group, XButton, Cell, LoanContract
    ...
    computed: {
      scrollerHeight () {
        return document.documentElement.clientHeight - this.btmContHeight - 48 + ''
      },
    },
    watch: {},
    mounted () {
